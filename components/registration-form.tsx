...
import { useFetchFormStructure } from "@/data/get-forms";
import React, { useState } from "react";
import { useForm } from "react-hook-form";

const RegistrationForm = ({ formId }: { formId: string }) => {
  const { data: formStructure } = useFetchFormStructure(formId);
  ...
  if (!formStructure)
    return (
      <div className="flex items-center justify-center min-h-screen">
        <span className="text-lg text-gray-800">جاري التحميل...</span>
      </div>
    );
